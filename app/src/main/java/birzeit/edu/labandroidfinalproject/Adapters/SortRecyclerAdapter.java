...
public class SortRecyclerAdapter extends RecyclerView.Adapter<SortRecyclerAdapter.ViewHolder> {
    ...
    public SortRecyclerAdapter(List<Destination> destinations) {
        this.destinations = destinations;
    }


    @Override
    public SortRecyclerAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        CardView v = (CardView) LayoutInflater.from(parent.getContext()).inflate(R.layout.destination_row, parent, false);

        return new SortRecyclerAdapter.ViewHolder(v);
    }

    @Override
    public void onBindViewHolder(SortRecyclerAdapter.ViewHolder holder, int position) {
        ...
        ImageView imageView = (ImageView) cardView.findViewById(R.id.img_country2);
        imageView.setImageResource(R.drawable.save_money);
        txt.setText(destinations.get(position).getCity());
        txt2.setText(destinations.get(position).getCost());
